...
export default function Organization() {
  ...
  useEffect(() => {
    ...
    listMembers(activeOrgId).then(setMembers).catch(() => {});
  }, [activeOrgId]);
  ...
  const onRoleChange = async (membershipId, role) => {
    ...
    setMembers((ms) => ms.map((m) => (m.id === membershipId ? { ...m, role } : m)));
  };

  const onRemove = async (membershipId) => {
    if (!confirm("Remove this member?")) return;
    await removeMember(activeOrgId, membershipId);
    setMembers((ms) => ms.filter((m) => m.id !== membershipId));
  };
              ...
              <tbody>
                {members.map((m) => (
                  <tr key={m.id}>
                    ...
                    <td>
                      <button className="link-btn danger" onClick={() => onRemove(m.id)}>
                        Remove
                      </button>
                    </td>
                  </tr>
